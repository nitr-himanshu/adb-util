    ...
    def __post_init__(self):
        ...
        try:
            ...
            self.parsed_time = datetime.strptime(
                f"2024-{self.timestamp}", "%Y-%m-%d %H:%M:%S.%f"
            )
        except ValueError:
...
class LogcatHandler:
    ...
    def __init__(self, device_id: str):
        ...
        self.logcat_patterns = [
            ...
            re.compile(
                r"^(\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3})\s+([VDIWEF])/([^(]+)\(\s*(\d+)\):\s*(.*)$"
            ),
        ]

        self.logger.info(f"Logcat handler initialized for device: {device_id}")

    async def start_logcat_stream(
        self,
            ...
            while self.is_streaming and self.logcat_process:
                try:
                    ...
                    line = await asyncio.wait_for(
                        self.logcat_process.stdout.readline(), timeout=0.5
                    )
                    ...
                    if line_str:
                        ...
                        if log_entry:
                            ...
                            if len(self.log_entries) > self.max_buffer_size:
                                self.log_entries = self.log_entries[
    ...
    def filter_entries(
        ...
        if tag_filter:
            if use_regex:
                try:
                    pattern = re.compile(
                        tag_filter, re.IGNORECASE if not case_sensitive else 0
                    )
                    filtered = [e for e in filtered if pattern.search(e.tag)]
                except re.error:
                    # Invalid regex, fall back to string search
                    tag_filter_lower = (
                        tag_filter.lower() if not case_sensitive else tag_filter
                    )
                    filtered = [
                        e
                        for e in filtered
                        if (
                            tag_filter_lower in e.tag.lower()
                            if not case_sensitive
                            else tag_filter in e.tag
                        )
                    ]
            else:
                tag_filter_lower = (
        ...
        if message_filter:
            if use_regex:
                try:
                    pattern = re.compile(
                        message_filter, re.IGNORECASE if not case_sensitive else 0
                    )
                    filtered = [e for e in filtered if pattern.search(e.message)]
                except re.error:
                    # Invalid regex, fall back to string search
                    msg_filter_lower = (
                        message_filter.lower() if not case_sensitive else message_filter
                    )
                    filtered = [
                        e
                        for e in filtered
                        if (
                            msg_filter_lower in e.message.lower()
                            if not case_sensitive
                            else message_filter in e.message
                        )
                    ]
            else:
                msg_filter_lower = (
    ...
    def get_buffer_stats(self) -> Dict[str, int]:
        ...
        return {"total": len(self.log_entries), "levels": level_counts}

    def export_logs(
        self, filename: str, entries: Optional[List[LogEntry]] = None
    ) -> bool:
        """Export logs to file."""
        try:
            ...
            self.logger.info(
                f"Exported {len(entries_to_export)} log entries to {filename}"
            )
            return True
