...
class FileOperations:
    ...
    async def push_file(
        self, local_path: Path, device_path: str, progress_callback=None
    ) -> bool:
        """Push file from local system to device."""
        try:
            self.logger.info(
                f"Pushing file {local_path} to {device_path} on device {self.device_id}"
            )
    ...
    async def pull_file(
        self, device_path: str, local_path: Path, progress_callback=None
    ) -> bool:
        """Pull file from device to local system."""
        try:
            self.logger.info(
                f"Pulling file {device_path} from device {self.device_id} to {local_path}"
            )
    ...
    async def delete_directory(self, device_path: str) -> bool:
        ...
        try:
            self.logger.info(
                f"Deleting directory {device_path} on device {self.device_id}"
            )
    ...
    async def create_directory(self, device_path: str) -> bool:
        ...
        try:
            self.logger.info(
                f"Creating directory {device_path} on device {self.device_id}"
            )
    ...
    async def move_file(self, source_path: str, dest_path: str) -> bool:
        ...
        try:
            self.logger.info(
                f"Moving file {source_path} to {dest_path} on device {self.device_id}"
            )
    ...
    async def copy_file(self, source_path: str, dest_path: str) -> bool:
        ...
        try:
            self.logger.info(
                f"Copying file {source_path} to {dest_path} on device {self.device_id}"
            )
    ...
    async def list_directory(self, device_path: str) -> List[FileInfo]:
        ...
        try:
            self.logger.info(
                f"Listing directory {device_path} on device {self.device_id}"
            )
            ...
            if process.returncode == 0:
                ...
                if files:  # If we got files, return them
                    self.logger.info(
                        f"Found {len(files)} items in directory using ls -la"
                    )
                    return files
            ...
            if process_simple.returncode == 0:
                ...
                self.logger.info(
                    f"Found {len(files)} items in directory using simple ls"
                )
                return files
            else:
                error_msg = stderr_simple.decode() if stderr_simple else "Unknown error"
                self.logger.error(
                    f"Failed to list directory with simple ls: {error_msg}"
                )
                return []
            ...
            if process.returncode == 0:
                ...
                if output:
                    ...
                    if len(parts) >= 8:
                        ...
                        try:
                            size = (
                                int(parts[4]) if not permissions.startswith("d") else 0
                            )
