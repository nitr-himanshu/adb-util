...
from ..core.commands.runner import ADBCommandRunner
...
class ScriptExecutionWorker(QThread):
    ...
    def __init__(
        self, script: Script, execution: ScriptExecution, device_id: str = None
    ):
        super().__init__()
        self.script = script
    ...
    def _execute_host_script(self):
        ...
        if self.script.script_type == ScriptType.HOST_WINDOWS:
            if os.name == "nt":
                ...
                raise RuntimeError(
                    "Cannot execute Windows batch file on non-Windows system"
                )
        else:  # HOST_LINUX
            if os.name == "nt":
    ...
    def _execute_device_script(self):
        ...
        try:
            ...
            mkdir_result = adb_runner.run_command(
                ["shell", "mkdir", "-p", "/sdcard/adb_util_scripts"]
            )
            ...
            push_result = adb_runner.run_command(
                ["push", str(script_path), device_script_path]
            )
            if not push_result.success:
                raise RuntimeError(
                    f"Failed to push script to device: {push_result.stderr}"
                )
            ...
            chmod_result = adb_runner.run_command(
                ["shell", "chmod", "+x", device_script_path]
            )
    ...
    def _run_process(self, cmd: List[str], cwd: Optional[Path] = None):
        ...
        try:
            ...
            while True:
                ...
                if output:
                    self.output_received.emit(
                        self.execution.execution_id, output.rstrip()
                    )

                error = self.process.stderr.readline()
                if error:
                    self.error_received.emit(
                        self.execution.execution_id, error.rstrip()
                    )
                ...
                if self.process.poll() is not None:
                    ...
                    if remaining_out:
                        self.output_received.emit(
                            self.execution.execution_id, remaining_out.rstrip()
                        )
                    if remaining_err:
                        self.error_received.emit(
                            self.execution.execution_id, remaining_err.rstrip()
                        )
                    break
...
class ScriptManager(QObject):
    ...
    def get_executions_for_script(self, script_id: str) -> List[ScriptExecution]:
        ...
        return [
            exec for exec in self.executions.values() if exec.script_id == script_id
        ]

    def get_recent_executions(self, limit: int = 10) -> List[ScriptExecution]:
        """Get recent executions."""
        sorted_executions = sorted(
            self.executions.values(), key=lambda x: x.start_time, reverse=True
        )
    ...
    def _on_execution_finished(self, execution_id: str, exit_code: int):
        ...
        self.logger.info(
            f"Script execution finished: {execution_id} (exit code: {exit_code})"
        )
    ...
    def cleanup_old_executions(self, max_age_days: int = 30, max_count: int = 100):
        ...
        if len(self.executions) > max_count:
            sorted_executions = sorted(
                self.executions.items(), key=lambda x: x[1].start_time, reverse=True
            )
        ...
        if executions_to_remove:
            self.logger.info(
                f"Cleaned up {len(executions_to_remove)} old execution records"
            )

    def export_scripts_to_json(
        self, file_path: str, script_ids: List[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            for script_data in import_data:
                try:
                    ...
                    if not all(
                        key in script_data for key in ["name", "script_path", "type"]
                    ):
                        self.logger.warning(
                            f"Skipping script with missing required fields: {script_data.get('name', 'Unknown')}"
                        )
                        skipped_count += 1
                        continue
                    ...
                    if not script_content:
                        if script_type_str == "host_windows":
                            ...
                            script_content = '#!/system/bin/sh\necho "Script: {}"\necho "Please edit this script to add your commands"'.format(
                                script_name
                            )
                        else:  # host_linux
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Invalid script type '{script_type_str}' for script '{script_name}'"
                        )
                        skipped_count += 1
                        continue
                    ...
                    for script in self.scripts.values():
                        if (
                            ...
                            and script.script_type == script_type
                        ):
                    ...
                    if existing_script:
                        ...
                        existing_script.is_template = script_data.get(
                            "isTemplate", False
                        )
                    ...
                    self.logger.error(
                        f"Failed to import script {script_data.get('name', 'Unknown')}: {e}"
                    )
                    skipped_count += 1
