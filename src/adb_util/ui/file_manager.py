...
from PyQt6.QtCore import QDir, Qt, QThread, QTimer, pyqtSignal
...
from ..services.live_editor import LiveEditorService
from ..utils.logger import get_logger, log_file_operation
from .integrated_text_editor import IntegratedTextEditor
...
class FileTransferWorker(QThread):
    ...
    def __init__(
        self, operation: str, file_ops: FileOperations, source: str, destination: str
    ):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                message = (
                    f"{self.operation.title()} completed successfully"
                    if result
                    else f"{self.operation.title()} failed"
                )
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Testing device connection before listing {self.path}"
                )
                ...
                if not connection_ok:
                    self.listing_failed.emit(
                        f"Cannot connect to device {self.file_ops.device_id}"
                    )
                    return

                # Now try to list the directory
                self.logger.info(f"Listing directory {self.path}")
                files = loop.run_until_complete(self.file_ops.list_directory(self.path))
            ...
            try:
                success = loop.run_until_complete(
                    self.live_editor.start_live_edit_session(
                        self.file_info, self.file_ops, self.editor_command
                    )
                )
                ...
                if success:
                    ...
                    self.session_failed.emit(
                        self.file_info.path, "Failed to start editing session"
                    )
...
class FileManager(QWidget):
    ...
    def create_toolbar(self):
        ...
        device_label.setStyleSheet(
            "color: #4CAF50;"
        )  # Green color to make it stand out
        device_label.setToolTip(
            f"Full Device ID: {self.device_id}"
        ...
        refresh_btn.setSizePolicy(
            refresh_btn.sizePolicy().horizontalPolicy(),
            refresh_btn.sizePolicy().verticalPolicy(),
        )
        ...
        new_folder_btn.setSizePolicy(
            new_folder_btn.sizePolicy().horizontalPolicy(),
            new_folder_btn.sizePolicy().verticalPolicy(),
        )
        ...
        home_btn.setSizePolicy(
            home_btn.sizePolicy().horizontalPolicy(),
            home_btn.sizePolicy().verticalPolicy(),
        )
        ...
        root_btn.setSizePolicy(
            root_btn.sizePolicy().horizontalPolicy(),
            root_btn.sizePolicy().verticalPolicy(),
        )
        ...
        test_btn.setSizePolicy(
            test_btn.sizePolicy().horizontalPolicy(),
            test_btn.sizePolicy().verticalPolicy(),
        )
    ...
    def create_local_panel(self):
        ...
        self.local_path_combo.lineEdit().returnPressed.connect(
        ...
        self.local_tree.setSelectionMode(
    ...
    def create_device_panel(self):
        ...
        self.device_path_combo.lineEdit().returnPressed.connect(
            self.navigate_device_folder
        )
    ...
    def load_device_directory(self):
        ...
        self.listing_worker = DirectoryListWorker(
            self.file_ops, self.current_device_path
        )
    ...
    def on_device_listing_completed(self, files: List[FileInfo]):
        ...
        self.progress_label.setText(
            f"Loaded {actual_count} items from {self.current_device_path}"
        )
    ...
    def on_device_listing_failed(self, error_message: str):
        ...
        QMessageBox.warning(
            self, "Directory Error", f"Failed to load directory:\n{error_message}"
        )
    ...
    def browse_local_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Folder", self.current_local_path
        )
        if folder:
    ...
    def upload_selected_files(self):
        ...
        if not selected_files:
            QMessageBox.information(
                self, "No Selection", "Please select files to upload."
            )
            return
    ...
    def sync_folders(self):
        ...
        QMessageBox.information(
            self, "Sync Folders", "Folder sync feature coming soon!"
        )
    ...
    def delete_local_file(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.progress_label.setText(
                    f"Deleted {len(selected_files)} local file(s)"
                )
                # Refresh local view
                ...
                QMessageBox.critical(
                    self, "Delete Error", f"Failed to delete files: {str(e)}"
                )
                    ...
                    try:
                        ...
                        if result:
                            self.test_completed.emit(
                                True, "Device connection successful"
                            )
                        else:
    ...
    def refresh_files(self):
        ...
        if hasattr(self, "local_model") and self.local_model:
            self.local_tree.setRootIndex(
                self.local_model.index(self.current_local_path)
            )
    ...
    def start_file_transfer(self, operation: str, source: str, destination: str):
        ...
        self.transfer_worker = FileTransferWorker(
            operation, self.file_ops, source, destination
        )
    ...
    def on_transfer_completed(self, success: bool, message: str):
        ...
        # Log the operation
        operation = (
            self.transfer_worker.operation if self.transfer_worker else "unknown"
        )
        status = "completed" if success else "failed"
        log_file_operation(
            operation,
            self.transfer_worker.source if self.transfer_worker else "",
            self.transfer_worker.destination if self.transfer_worker else "",
            status,
        )
    ...
    def populate_device_path_combo(self):
        ...
        if bookmarks:
            for bookmark in bookmarks:
                self.device_path_combo.addItem(
                    f"⭐ {bookmark['name']}", bookmark["path"]
                )
    ...
    def navigate_to_local_path(self, path: str):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Navigation Error", f"Could not navigate to: {path}"
            )
    ...
    def bookmark_local_path(self):
        ...
        name, ok = QInputDialog.getText(
            self, "Add Bookmark", "Enter bookmark name:", text=Path(current_path).name
        )
    ...
    def bookmark_device_path(self):
        ...
        # Get bookmark name from user
        path_name = (
            Path(current_path).name or current_path.split("/")[-1] or current_path
        )
        name, ok = QInputDialog.getText(
            self, "Add Bookmark", "Enter bookmark name:", text=path_name
        )
    ...
    def open_device_file_with_editor(self):
        ...
        try:
            ...
            if not selected_files or len(selected_files) != 1:
                QMessageBox.information(
                    self, "Invalid Selection", "Please select exactly one file to edit."
                )
                return
            ...
            if file_info.is_directory:
                QMessageBox.information(
                    self,
                    "Invalid Selection",
                    "Cannot edit directories. Please select a file.",
                )
                return
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to open file with editor: {str(e)}"
            )
    ...
    def select_editor(self, available_editors) -> Optional[str]:
        ...
        if choice == "Custom Command...":
            ...
            command, ok = QInputDialog.getText(
                self, "Custom Editor Command", "Enter the command to open files:"
            )
    ...
    def start_live_edit_worker(self, file_info: FileInfo, editor_command: str):
        ...
        try:
            ...
            self.live_edit_worker = LiveEditWorker(
                file_info, self.file_ops, self.live_editor, editor_command
            )
            self.live_edit_worker.session_started.connect(
                self.on_live_edit_worker_started
            )
            self.live_edit_worker.session_failed.connect(
                self.on_live_edit_worker_failed
            )
    ...
    def on_live_edit_worker_failed(self, device_path: str, error_message: str):
        ...
        self.logger.error(
            f"Worker failed to start editing {device_path}: {error_message}"
        )
        QMessageBox.critical(
            self,
            "Live Edit Error",
    ...
    def on_live_edit_ended(self, device_path: str, success: bool):
        ...
        if success:
            self.progress_label.setText(
                f"✅ Finished editing {filename} - changes saved"
            )
    ...
    def on_live_edit_error(self, device_path: str, error_message: str):
        ...
        QMessageBox.critical(
            self, "Live Edit Error", f"Error editing {filename}:\n{error_message}"
        )
    ...
    def open_device_file_with_integrated_editor(self):
        ...
        try:
            ...
            # Show progress
            self.progress_label.setText(
                f"📝 Opening {file_info.name} in integrated editor..."
            )
            ...
            if result == QDialog.DialogCode.Accepted:
                self.logger.info(
                    f"Integrated editor session completed for {file_info.path}"
                )

        except Exception as e:
            self.logger.error(f"Error opening file with integrated editor: {e}")
            QMessageBox.critical(
                self, "Error", f"Failed to open file with integrated editor: {str(e)}"
            )
            self.progress_label.setText("Ready")
