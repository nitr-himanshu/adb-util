...
class CodeEditor(QPlainTextEdit):
    ...
    def highlight_current_line(self):
        ...
        if not self.isReadOnly():
            ...
            app_stylesheet = app.styleSheet() if app else ""
            is_dark_theme = (
                "#2b2b2b" in app_stylesheet or "dark" in app_stylesheet.lower()
            )
            ...
            if is_dark_theme:
                ...
                line_color = QColor(
                    "#2a2a2a"
                )  # Very subtle gray, barely darker than background
            else:
...
class IntegratedTextEditor(QDialog):
    ...
    def __init__(self, file_info: FileInfo, file_ops: FileOperations, parent=None):
        ...
        detected_theme = (
            "dark"
            if "#2b2b2b" in app_stylesheet or "dark" in app_stylesheet.lower()
            else "light"
        )
    ...
    def setup_toolbar(self, layout):
        ...
        # File info
        info_label = QLabel(
            f"File: {self.file_info.path} | Size: {self.file_info.size} bytes"
        )
    ...
    def save_file(self, show_message=True):
        ...
        self.upload_worker = FileUploadWorker(
            content, self.file_info.path, self.file_ops
        )
    ...
    def save_as_file(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save File As", self.file_info.name, "All Files (*.*)"
        )
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(
                    self, "Save Error", f"Failed to save local file:\n{str(e)}"
                )
    ...
    def format_json(self):
        ...
        try:
            ...
            formatted_json = json.dumps(
                parsed_json, indent=4, ensure_ascii=False, sort_keys=True
            )
            ...
            QMessageBox.critical(
                self, "Format Error", f"Failed to format JSON:\n{str(e)}"
            )
    ...
    def minify_json(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Minify Error", f"Failed to minify JSON:\n{str(e)}"
            )
        ...
        try:
            ...
            if not current_text:
                QMessageBox.information(
                    self, "JSON Validation", "No content to validate"
                )
            ...
            QMessageBox.critical(
                self, "Validation Error", f"Failed to validate JSON:\n{str(e)}"
            )
