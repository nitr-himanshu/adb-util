...
from ..core.commands.logcat_handler import LogcatHandler, LogEntry
...
class LogcatWorker(QThread):
    ...
    def set_parameters(
        self, buffer: str, format_type: str, filter_spec: Optional[str] = None
    ):
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(
                f"LogcatWorker thread started for device: {self.device_id}"
            )
...
class Logging(QWidget):
    ...
    def create_log_panel(self):
        ...
        self.buffer_size_spin.setMinimumWidth(
            120
        )  # Increase width for better visibility
        self.buffer_size_spin.setMinimumHeight(
            25
    ...
    def create_filter_panel(self):
        ...
        self.highlight_input.returnPressed.connect(
            self.add_highlight_keyword
    ...
    def start_capture(self):
        ...
        try:
            ...
            try:
                ...
                self.logcat_worker.log_entry_received.connect(
                    self.on_log_entry_received
                )
                self.logcat_worker.stream_status_changed.connect(
                    self.on_stream_status_changed
                )
    ...
    def _cleanup_worker(self):
        ...
        try:
            ...
            if self._stop_attempts < 10:  # Wait up to 1 second (10 * 100ms)
                ...
                self.logger.warning(
                    "Worker thread didn't stop gracefully, terminating..."
                )
    ...
    def update_entry_count(self):
        ...
        self.entry_count_label.setText(
            f"Entries: {len(self.log_entries)} (Filtered: {len(self.filtered_entries)})"
        )
    ...
    def on_logcat_error(self, error_msg):
        ...
        QMessageBox.warning(
            self, "Logcat Error", f"Logcat error occurred:\n{error_msg}"
        )
        self.stop_capture()
    ...
    def display_log_entry(self, entry):
        ...
        if entry.tag == "raw" or (not entry.timestamp and not entry.pid):
            ...
            log_line = (
                f"{timestamp_part}{entry.level}/{entry.tag}{pid_part}: {entry.message}"
            )
        ...
        if highlight_keywords:
            ...
            self._insert_highlighted_text(
                cursor, log_line, entry.level, colors, highlight_keywords
            )
        else:
        ...
        if filename:
            try:
                entries_to_save = (
                    ...
                    if self.export_filtered_only.isChecked()
                ...
                if success:
                    QMessageBox.information(
                        self, "Export Successful", f"Logs exported to {filename}"
                    )
    ...
    def search_logs(self):
        ...
        if search_text:
            ...
            self.status_label.setText(
                "Status: Capturing..." if self.is_capturing else "Status: Ready"
            )
    ...
    def apply_filters_optimized(self):
        ...
        if self.search_input.text():
            self.status_label.setText(
                f"Status: Found {len(self.filtered_entries)} matches"
            )
        else:
            self.status_label.setText(
                "Status: Capturing..." if self.is_capturing else "Status: Ready"
    ...
    def refresh_display(self):
        ...
        if len(self.filtered_entries) > 100:
            ...
            self.log_display.append(
                "... (showing last 100 entries for performance) ...\n"
            )
        else:
    ...
    def add_highlight_keyword(self, show_dialogs=True):
        ...
        if keyword:
            ...
            if self.filtered_entries and len(self.filtered_entries) < 50:
                ...
                self.logger.info(
                    f"Keyword '{keyword}' will be highlighted in new log entries"
                )
    ...
    def export_logs(self):
        ...
        if filename:
            try:
                ...
                success = self.logcat_handler.export_logs(
                    filename, self.filtered_entries
                )
        ...
        try:
            ...
            if hasattr(self, "log_display") and self.log_display:
                ...
                for group in [
                    ...
                    palette.setColor(
                        group, QPalette.ColorRole.WindowText, QColor(text_color)
                    )
                ...
                if hasattr(self.log_display, "viewport"):
                    ...
                    if viewport:
                        ...
                        viewport_palette.setColor(
                            QPalette.ColorRole.Base, QColor(bg_color)
                        )
                        viewport_palette.setColor(
                            QPalette.ColorRole.Text, QColor(text_color)
                        )
                ...
                self.logger.info(
                    f"Applied {theme_name} theme: bg={bg_color}, text={text_color}"
                )
            ...
            button_attributes = ["start_btn", "stop_btn", "clear_btn", "save_btn"]
            ...
            self.logger.info(
                f"Logging theme comprehensively refreshed to {theme_name} mode - ALL elements styled"
            )
    ...
    def refresh_theme_optimized(self):
        ...
        try:
            ...
            if hasattr(self, "highlight_input") and self.highlight_input:
                self.highlight_input.setStyleSheet(
                    f"""
                QLineEdit {{
                    background-color: {bg_color} !important;
                    color: {text_color} !important;
    ...
    def _insert_highlighted_text(
        self, cursor, text, log_level, colors, highlight_keywords
    ):
        ...
        while remaining_text:
            ...
            if earliest_match:
                ...
                cursor.insertText(
                    remaining_text[earliest_match.start() : earliest_match.end()]
                )
