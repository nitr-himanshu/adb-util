...
class MainWindow(QMainWindow):
    ...
    def on_devices_discovered(self, devices):
        ...
        self.device_count_label.setText(
            f"Devices: {len(devices)} ({connected_count} online)"
        )
        self.status_label.setText(
            f"Found {len(devices)} devices, {connected_count} online"
        )
    ...
    def open_device_tab(self, mode):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create {mode} tab for device {device_id}: {e}",
                exc_info=True,
            )
            QMessageBox.critical(
                self, "Error", f"Failed to create {mode} tab: {str(e)}"
            )
        ...
        try:
            ...
            for i in range(self.tab_widget.count()):
                ...
                if (
                    ...
                    and widget.__class__.__name__ == "ScriptManagerTab"
                ):
            ...
            from .script_manager_tab import ScriptManagerTab
            ...
            self.logger.error(
                f"Failed to create script manager tab: {e}", exc_info=True
            )
            QMessageBox.critical(
                self, "Error", f"Failed to create script manager tab: {str(e)}"
            )
    ...
    def init_theme_manager(self):
        ...
        try:
            ...
            self.logger.info(
                f"Theme manager initialized with {theme_manager.get_current_theme()} theme"
            )
        ...
        try:
            ...
            if hasattr(self, "theme_toggle_btn"):
                ...
                self.theme_toggle_btn.setToolTip(
                    f"Switch to {theme_text.lower()} (Ctrl+T)"
                )
