...
class ScriptSyntaxHighlighter(QSyntaxHighlighter):
    ...
    def setup_highlighting_rules(self):
        ...
        if self.script_type == ScriptType.HOST_WINDOWS:
            ...
            self.highlighting_rules.append(
                (
                    re.compile(r"\\brem\\b.*$", re.IGNORECASE | re.MULTILINE),
                    comment_format,
                )
            )
            self.highlighting_rules.append(
                (re.compile(r"^::.*$", re.MULTILINE), comment_format)
            )
            ...
            # Variables ($VAR, ${VAR})
            self.highlighting_rules.append(
                (re.compile(r"\\$\\{?[A-Za-z_][A-Za-z0-9_]*\\}?"), variable_format)
            )
...
class ScriptCodeEditor(QPlainTextEdit):
    ...
    def line_number_area_paint_event(self, event):
        ...
        # Get theme colors
        if (
            hasattr(theme_manager, "get_current_theme")
            and theme_manager.get_current_theme() == "dark"
        ):
...
class ScriptEditorDialog(QDialog):
    ...
    def __init__(self, script: Optional[Script] = None, parent=None):
        ...
        self.setWindowTitle(
            "Script Editor" if self.is_new_script else f"Edit Script: {script.name}"
        )
        self.setModal(True)
        self.resize(900, 700)
    ...
    def create_properties_section(self) -> QWidget:
        ...
        self.is_template_check.setToolTip(
            "Template scripts can be used as starting points for new scripts"
        )
        if not self.is_new_script:
        ...
        self.is_visible_check.setToolTip(
            "Uncheck to hide this script from the main list"
        )
...
df | grep -E "/data|/system"''',
        ...
        if not available_templates:
            QMessageBox.information(
                self, "No Templates", "No templates available for this script type."
            )
            return
    ...
    def load_script_content(self):
        ...
        if self.script and Path(self.script.script_path).exists():
            try:
                ...
                QMessageBox.critical(
                    self, "Load Error", f"Failed to load script content:\n{e}"
                )
    ...
    def update_title(self):
        ...
        base_title = (
            "Script Editor"
            if self.is_new_script
            else f"Edit Script: {self.script.name}"
        )
    ...
    def save_script(self):
        ...
        try:
                ...
                if str(script_path) != self.script.script_path:
                    ...
                    if old_path.exists() and old_path != script_path:
                        try:
                            ...
                            self.logger.warning(
                                f"Could not remove old script file: {e}"
                            )
            ...
            QMessageBox.information(
                self, "Success", f"Script saved successfully to:\n{script_path}"
            )
    ...
    def save_as_script(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Script saved to:\n{file_path}"
                )
    ...
    def validate_input(self) -> bool:
        ...
        if not content:
            QMessageBox.warning(
                self, "Validation Error", "Please enter script content."
            )
            self.editor.setFocus()
            return False
