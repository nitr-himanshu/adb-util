...
from ..core.commands.runner import CommandRunner
...
class Terminal(QWidget):
    ...
    def create_saved_commands_panel(self):
        ...
        self.category_combo.addItems(
            ["All", "System", "Files", "Process", "Network", "Custom"]
        )
        ...
        self.saved_commands_list.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.saved_commands_list.customContextMenuRequested.connect(
            self.show_command_context_menu
        )
        ...
        for name, command in quick_commands:
            ...
            btn.clicked.connect(
                lambda checked, cmd=command: self.execute_quick_command(cmd)
            )
    ...
    def initialize_terminal(self):
        ...
        self.add_terminal_output(
            f"Session started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "system"
        )
    ...
    def auto_complete(self):
        ...
        if len(matches) == 1:
            ...
            self.add_terminal_output(
                f"Possible completions: {', '.join(matches)}", "info"
            )
    ...
    def execute_command(self):
        ...
        if self.is_command_running:
            QMessageBox.warning(
                self,
                "Command Running",
                "Please wait for the current command to finish.",
            )
            return
    ...
    def handle_builtin_command(self, command):
        ...
        if cmd == "help":
            ...
            self.add_terminal_output(
                f"Current directory: {self.current_directory}", "info"
            )
            return True
...
Use Tab for auto-completion
    ...
    def on_command_finished(self, stdout, stderr, return_code):
        ...
        if return_code != 0 and not stderr:
            self.add_terminal_output(
                f"Command exited with code: {return_code}", "error"
            )
        ...
        if filename:
            try:
                ...
                QMessageBox.information(
                    self, "Session Saved", f"Terminal session saved to:\n{filename}"
                )
            ...
            if current_category == "All" or current_category == category:
                for cmd_data in commands:
                    item = QListWidgetItem(
                        f"{cmd_data['name']} - {cmd_data['command']}"
                    )
    ...
    def edit_saved_command(self):
        ...
        name, ok = QInputDialog.getText(
            self, "Edit Command", "Command name:", text=cmd_data["name"]
        )
        if not ok:
            return

        command, ok = QInputDialog.getText(
            self, "Edit Command", "Command:", text=cmd_data["command"]
        )
        if not ok:
            return
